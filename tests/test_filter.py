...
def test_min_trades_fails_below_threshold():
    t = _make_trader(num_trades_30d=4)
...
def test_min_pnl_passes_when_above_threshold():
    t = _make_trader(total_pnl_30d=600.0)
    assert _passes_min_pnl(t) is True


def test_min_pnl_fails_below_threshold():
    t = _make_trader(total_pnl_30d=400.0)
    assert _passes_min_pnl(t) is False


def test_min_vol_passes_when_above_threshold():
    t = _make_trader(vol=1500.0)
    assert _passes_min_vol(t) is True


def test_min_vol_fails_below_threshold():
    t = _make_trader(vol=900.0)
    assert _passes_min_vol(t) is False


def test_min_open_positions_passes_at_threshold():
    t = _make_trader(num_open_positions=1)
    assert _passes_min_open_positions(t) is True


def test_min_open_positions_fails_below_threshold():
    t = _make_trader(num_open_positions=0)
    assert _passes_min_open_positions(t) is False
